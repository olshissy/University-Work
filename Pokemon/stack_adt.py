...
class ArrayStack(Stack[T]):
    ...
    def __str__(self) -> str:
        """
        The _str_ method returns a string of all the _str_ methods of the objects in self.
        Pre-condition: None
        Post-condition: None
        Time complexity:
        --> Best case and Worst case: O(n) where n is the length of self, as we iterate through.
        """
        if not self.is_empty():
            string = ""
            for i in range(len(self) - 1, -1, -1):
                if i == 0:
                    string += f"{str(self.array[i])}"
                else:
                    string += f"{str(self.array[i])},"
            return string
        else:
